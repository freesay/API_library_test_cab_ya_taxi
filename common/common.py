...
class Common:
    ...
    def get_response(self, filename, postfix):
        """
    ...
    def del_response(self, filename, postfix, data=''):
        """
        Метод собирает все необходимые данные для DELETE запроса и сохраняет полученные данные в файл JSON.
        """
        method = "DELETE"
        url = self.head_url + postfix
        response = requests.request(method, url, headers=self.headers_auth, data=data, verify=self.verify)
